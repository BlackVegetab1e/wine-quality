...
class C45():
    def __init__(self, data_X, data_Y, epsilon, alpha):
    ...
    def generate_tree(self, now_node:node, feature_index:np.ndarray):
        ...
        for i in feature_index:
            ...
            step = int(f_N/10)
            for j in range(1,10):
                feature_cut = classes_[step*j]
                g_r= g_contious_ratio(now_node.data_X, now_node.data_Y, i, feature_cut)
                if g_r > g_r_max :
        ...
        print(np.where(feature_index == best_feature_index)[0])
...
if __name__ == "__main__":
    ...
    print(training_y)
    c45 = C45(training_x, training_y, 0.011, 0.02)
